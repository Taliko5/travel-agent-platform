...
### D3 — `trace_id`/`span_id` stay null outside a span; no attempt to change that
`TraceCorrelationFilter` already does the right thing here, and the scaffold's own `Structured Logging with Trace Correlation` spec says so explicitly (`Scenario: A log is emitted outside any active span`: fields present but null, not an error). Uvicorn's access line is logged after the ASGI app has returned control to it — after the request's span has already ended, per pass 1b's finding — so a null `trace_id` on that specific line is the expected outcome of D2's mechanism, not a gap D2 leaves open. Making it non-null would require capturing the request's span context somewhere that survives past the span's own lifetime (request-scoped middleware storing it for later use, or similar), which the proposal this design serves explicitly rules out.
