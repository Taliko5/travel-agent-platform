...
- [ ] 3.4 Push a docs-only test commit to a scratch branch and confirm all four checks still report success (not skipped/absent) with no real work performed — requires pushing to GitHub and observing Actions; not done in this session.
...
- [ ] 4.2 Confirm the job passes in CI (not just locally) — requires pushing to GitHub and observing Actions; not done in this session.
...
- [ ] 5.3 Confirm both `build-backend` and `build-frontend` pass in CI — requires pushing to GitHub and observing Actions; not done in this session.
...
- [ ] 8.4 Confirm scan results appear in the Security tab after a PR run, and confirm the build jobs do not fail regardless of findings (non-blocking by design) — requires pushing to GitHub and observing Actions; not done in this session.
...
- [ ] 9.2 Confirm the job passes on the current repo state (no findings expected, since `backend/.env` is already gitignored and untracked) — requires pushing to GitHub and observing Actions; not done in this session.
