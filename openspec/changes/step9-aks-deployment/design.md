...
**Mechanism note (task 5.5): `skip_service_principal_aad_check = true` on `kubelet_acr_pull`.** `azurerm_role_assignment.kubelet_acr_pull` (`cluster/registry.tf`) grants `AcrPull` to `azurerm_kubernetes_cluster.this.kubelet_identity[0].object_id` — a managed identity Azure creates as part of the same `terraform apply` that creates the cluster, seconds before the role assignment that targets it. Entra ID replication lag between an identity's creation and its object becoming queryable by the role-assignment API routinely fails that request with `PrincipalNotFound` in exactly this pattern — a role assigned to a principal created moments earlier in the same apply. `azurerm_role_assignment` has an argument for precisely this case, which skips the provider's pre-flight existence check and lets the assignment succeed on the first try rather than depend on retry/propagation timing.
...
Quoted from [Use Azure RBAC for Kubernetes Authorization](https://learn.microsoft.com/en-us/azure/aks/manage-azure-rbac): Cluster User "allows access to the Kubernetes API server (required to use `kubectl`)" — this is the credential-retrieval role `az aks get-credentials` (task 6.4) needs, and it grants nothing about what the retrieved credential can then do inside the API server. RBAC Writer "allows read and write access to most objects in a namespace" — at the time this was written, that was taken to cover what `helm upgrade --install` needs to create and update the chart's Deployments, Services, ServiceAccount, `SecretProviderClass`, `Gateway` and `HTTPRoute` (Section 3), without the delete-anything, every-namespace reach of Cluster Admin. **That was an unverified assumption, and task 8.8's live deploy falsified it: Azure's built-in Kubernetes Authorization roles below Cluster Admin do not extend to custom resources (CRDs) at all** — `SecretProviderClass`, `Gateway` and `HTTPRoute` are all CRDs, so RBAC Writer was never going to cover them regardless of scope or propagation timing. See the new mechanism note below (task 5.9 extension) for what was actually observed and the decision taken.

**Rejected: "Azure Kubernetes Service Cluster Admin Role."** It grants unrestricted access to every namespace and every verb — more than a `helm upgrade --install` run from one narrowly-scoped GitHub Actions job needs. This is the same proportionality D4 already applies to the registry (`AcrPush`, not `Owner`) and D5 already applies to the backend identity (`Key Vault Secrets User`, not `Key Vault Administrator`); granting Cluster Admin here would work against that posture rather than extend it. **Superseded for CI specifically** — task 8.8's live deploy showed RBAC Writer cannot create the chart's CRDs at all, and Cluster Admin is what CI was actually granted as a result. The reasoning above is kept as the record of the least-privilege posture assumed at the time this decision was first made; see the new mechanism note below (task 5.9 extension) for why it didn't hold and what was decided instead.
...
**Mechanism note (task 5.9): the operator has no data-plane role either, and `azure_rbac_enabled = true` grants nothing by default.** Enabling Azure RBAC for Kubernetes Authorization (`cluster.tf`) moves Kubernetes authorization entirely to Azure RBAC `dataActions` — no identity gets `kubectl` access it isn't explicitly assigned. Subscription Owner, the role the operator authenticates as, is a control-plane role: it grants `Microsoft.ContainerService/managedClusters/listClusterUserCredential/action`, so `az aks get-credentials` succeeds, but no `Microsoft.ContainerService/managedClusters/*/read` (or write) `dataActions` — so every `kubectl` call that follows returns `Forbidden`. The only identity with a data-plane role is CI (`ci_aks_rbac_writer` above); nothing grants the operator one, so Section 9's verification tasks would lock the operator out of the cluster this design exists to stand up.

**Fix: `azurerm_role_assignment.operator_aks_rbac_cluster_admin`**, `Azure Kubernetes Service RBAC Cluster Admin` scoped to the cluster resource, principal `data.azurerm_client_config.current.object_id` — whoever runs `terraform apply` becomes the cluster admin. Under the current operational model (task 7.5.1: the cluster state is applied by hand from Cloud Shell every session, never by CI) that is always the owner.

**Cluster Admin, not Reader or Writer.** The operator's job across Section 9 includes `kubectl exec`, `port-forward`, checks against custom API groups a narrower built-in role doesn't cover (`gateway.networking.k8s.io` for the routing check, `secrets-store.csi.x-k8s.io` for the CSI check), and one-off debugging — none of it the single, predictable operation CI's `RBAC Writer` is scoped for. CI keeps the narrower role because CI only deploys.

**Not `--admin`.** `az aks get-credentials --admin` still works, because `local_account_disabled` is unset, but that fetches the static cluster-admin certificate — the standing bearer credential this design's OIDC premise (D4, D5) exists to avoid in the first place. The role assignment above keeps every `kubectl` call authenticating through the operator's own short-lived Entra token; it is what makes that token carry permissions inside the API server, rather than reintroducing the credential D4 already rejected once.

**Mechanism note (task 8.8): RBAC Writer cannot create the chart's CRDs; CI widened to Cluster Admin.** Task 8.8's actual `helm upgrade --install` run from CI failed with:

```
gateways.gateway.networking.k8s.io ... is forbidden: User ... does not have access to the resource in Azure
```

Checked against Microsoft's own docs ([Use Azure RBAC for Kubernetes Authorization](https://learn.microsoft.com/en-us/azure/aks/manage-azure-rbac), [Azure RBAC for Kubernetes Authorization](https://learn.microsoft.com/en-us/azure/aks/entra-id-authorization)): the built-in Azure RBAC for Kubernetes Authorization roles below Cluster Admin — Reader, Writer, Admin — do not extend to custom resources (CRDs) at all. Only a custom role carrying a `Microsoft.ContainerService/managedClusters/customresources/read` (and corresponding write) `dataAction` — optionally narrowed with ABAC conditions to scope it to specific resource types or namespaces — or "Azure Kubernetes Service RBAC Cluster Admin" (which carries a full wildcard) covers `SecretProviderClass`, `Gateway` and `HTTPRoute`. Since the chart creates all three (Section 3), CI's RBAC Writer was never going to satisfy this, regardless of role-propagation timing.
...
**Mechanism note: `operator_object_id` grants the write role RBAC would otherwise deny.** An RBAC-authorized vault (`enable_rbac_authorization = true`, `platform/keyvault.tf`) denies `az keyvault secret set` to everyone by default — the operator needs `Key Vault Secrets Officer` on the vault before that out-of-band write can succeed. `platform/keyvault.tf`'s optional `var.operator_object_id` grants exactly that role from Terraform, so the operator doesn't have to create the role assignment by hand first. Left `null` by default: if unset, nobody gets this grant from Terraform and the operator authorizes themselves separately.

**Mechanism note (task 4.3): no `azurerm_key_vault_secret` resource in this state.** `azurerm_key_vault_secret` always stores its `value` attribute in Terraform state, regardless of how that value is populated — a placeholder value would put something in state this section's Decision already says must never be there, and it would fight the out-of-band `az keyvault secret set` write on every subsequent `terraform plan`. `platform/keyvault.tf` therefore declares the vault only; the secret object named `var.key_vault_secret_object_name` is created entirely out-of-band, name and value both, by the operator via `az keyvault secret set` (`platform/README.md`).

**Mechanism note (task 4.6): no output resolves to a secret-shaped attribute either.** Every value `platform/outputs.tf` exposes resolves to `.id`, `.name`, `.location`, `.login_server`, `.tenant_id`, `.client_id` or `.principal_id` — identifiers and URLs, not the vault secret's value. This state never reads the secret (the point of the mechanism note above), so it has nothing secret-shaped to output even by mistake.

**Mechanism note (task 4.5): the platform state is applied twice.** The backend's federated identity credential (`azurerm_federated_identity_credential.backend_serviceaccount`, conditional on `var.aks_oidc_issuer_url`) needs the AKS cluster's OIDC issuer URL, which is an output of `Infrastructure/terraform/cluster/` — a state that doesn't exist on the platform state's first apply (D10's platform-before-cluster sequencing). The platform state is therefore applied twice: once before the cluster exists, with `aks_oidc_issuer_url` left at its `""` default (the federated credential is skipped, `count = 0`), and once again after the cluster state stands up, feeding `-var="aks_oidc_issuer_url=$(terraform -chdir=../cluster output -raw oidc_issuer_url)"` so the credential is actually created. The full three-step sequence is in `platform/README.md`; task 8.6.1 is where this runs during the raise, and — because the cluster's OIDC issuer URL is a random GUID unique to each cluster instance — it reruns on every cycle of this change's raise/teardown sequence (tasks.md 7.5.3), not just the first.
...
**What enabling it costs.** The unit price is read: $0.16 per 10M ingested samples. Turning that into a session or monthly figure needs an active-series count and a scrape interval — task 9.12 read both directly from the workspace's own ingestion metric on a raised cluster, replacing the arithmetic guess this paragraph originally carried. Full figures: `docs/step9-raise-evidence.md`, task 9.12.

The honest summary is that ingestion is small relative to $0.48/hour of nodes and load balancer — now a measured conclusion, not an estimate; see task 9.12 for the figure.
...
**Mechanism note (task 5.8): no `terraform_remote_state` from the cluster state into the platform state.** `Infrastructure/terraform/cluster/` takes the platform state's `acr_id` and `ci_identity_principal_id` as plain string-valued input variables (`cluster/variables.tf`), never as a live reference via `terraform_remote_state` — confirmed by `grep -rn "terraform_remote_state" Infrastructure/terraform/cluster/`, which returns nothing. Because neither value is a Terraform reference into the platform configuration, `terraform destroy` run against the cluster state has no path to anything the platform state created: the isolation "Two states, two paths" above describes holds structurally, not just organizationally.

**Mechanism note: state is local, no remote backend.** Neither state configures a Terraform backend (`platform/versions.tf`, `cluster/versions.tf`) — this design doesn't create a storage account to hold one, and adding one is a resource this pass never decided to raise. State lives wherever `apply`/`destroy` was actually run (Azure Cloud Shell under the current operational model, task 7.5.1); the repository's top-level `.gitignore` keeps `*.tfstate*`, `.terraform/` and `*.tfvars` out of the repository, so no state file — and nothing a state file can contain — ever reaches source control.
...
**Decided (task 3.10): a single-replica ReadWriteOnce PVC, not a standalone Chroma server.** The chart runs one backend replica (`values.yaml` `replicaCount.backend: 1`), which removes the reason a server would be needed — "multi-replica pods can't share a named volume" doesn't apply when there is one pod to begin with, and D14's capacity accounting already has no headroom budgeted for a second stateful workload alongside the Istio control plane, the CSI DaemonSets and managed Prometheus. Independent of that budget argument, `backend/rag/retriever.py`'s `Chroma(persist_directory=CHROMA_DIR, ...)` is an embedded, file-backed store not safe for concurrent access from multiple processes — so `replicaCount.backend > 1` isn't just expensive, it's unsupported by the current RAG implementation. Server mode would also touch `backend/rag/retriever.py` — `Chroma(persist_directory=CHROMA_DIR, ...)` becomes an `HttpClient` pointed at a service — which is an application-code change this step doesn't otherwise make (`proposal.md`'s Impact line: CORS and the frontend build argument are the only two). A PVC mounted at `rag/chroma_db` needs none of that: `CHROMA_DIR` stays a local path, `get_vectorstore()` is unchanged, and the four lines `docs/step5.md` already named as the only mode-dependent code stay untouched because this isn't a mode Chroma has to know about — it's just a volume.

**Ingestion is an initContainer on the backend Deployment, not a Helm hook Job.** `backend-deployment.yaml`'s `initContainers` runs `python rag/ingest.py` against the same PVC before the backend container starts — Kubernetes blocks a Pod's main containers until every initContainer exits 0, so ingestion completes before the backend ever serves traffic, on every pod start. This replaces a Helm hook Job (`helm.sh/hook: pre-install,pre-upgrade`) for a reason discovered rather than anticipated: that hook annotation runs the Job before *any* non-hook resource in the chart exists — including the ServiceAccount, `SecretProviderClass` and PVC the Job itself depends on, all defined in this same chart's first install, so a fresh install always failed on "serviceaccount not found" and timed out. Giving those three resources hook annotations too was considered and rejected: the PVC would need `hook-delete-policy: before-hook-creation` to be recreated on every upgrade, which means destroying and reprovisioning the Azure Disk holding the Chroma data on every deploy.

**The old claim "the two never mount concurrently" was only true for a fresh install.** On upgrade, the pre-upgrade hook ran while the *previous* release's backend pod was still alive and still holding the ReadWriteOnce PVC — a second, separate race from the RollingUpdate one, since the hook ran before Helm touched the Deployment at all. Moving ingestion into the Deployment's own initContainer removes this second race entirely — there's no longer a separate hook Job to compete with the old pod — collapsing it into the same old-pod-vs-new-pod race `strategy: Recreate` (`backend-deployment.yaml`) already resolves.

**Corpus-update tradeoff, accepted rather than tooled around.** `rag/ingest.py` now skips re-ingesting when the store already has documents (checked via `Chroma(persist_directory=CHROMA_DIR, ...).get(limit=1)`), so updating the files in `rag/data/` no longer propagates automatically to a running cluster's PVC on the next deploy. What survives a pod restart or a Deployment rollout without re-ingesting is the PVC itself — what does not survive is the PVC across a cluster teardown (D10), where the next raise starts from an empty store and ingestion runs again, which is the quota cost D10 already names.
...
**SKU correction (2026-09-16): `Standard_D4s_v7` → `Standard_D2s_v7`**, for a different reason than the correction above — not SKU-family unavailability, but this subscription's own quota ceiling. `az vm list-usage --location germanywestcentral` shows `Total Regional vCPUs` capped at 4 for the whole region, across every VM family combined (this subscription is an Azure Free Trial), so 2 nodes × `Standard_D4s_v7` (8 vCPU) exceeds it outright — confirmed by `terraform apply` failing with `ErrCode_InsufficientVCPUQuota`. A Free Trial subscription cannot self-serve a quota increase (must upgrade to Pay-As-You-Go first), so the correction is `Standard_D2s_v7` — 2 vCPU / 8 GiB, already confirmed creatable by task 8.5's own probe, and the only SKU that fits 2 nodes inside the 4-vCPU ceiling. This is a deliberate deviation below the 4 vCPU / 16 GiB floor stated above, driven entirely by this subscription's quota, not a revision of the sizing reasoning itself. Confirmed hourly price in `germanywestcentral`, Linux, Consumption, via the Azure Retail Prices API: `Standard_D2s_v7` = $0.152/hour. Recomputed: 2 nodes × $0.152 = $0.304/hour (was $0.61/hour), lowering the cost floor from ≈$0.674/hour to **≈$0.365/hour** ($0.304 node + $0.031 disk + $0.025 load balancer + $0.005 IP). Owner approved 2026-09-16.
