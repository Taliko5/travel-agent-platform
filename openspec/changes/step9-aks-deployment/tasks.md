...
- [ ] 6.7 Set `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AKS_CLUSTER_NAME` (`travel-agent`) and `AKS_RESOURCE_GROUP_NAME` (`travel-agent-cluster`) as repository Variables now, ahead of any apply — `design.md` D4's extension is explicit that none of the four depends on Terraform having run. Not done in this pass: setting live repository configuration is an operator action (`gh variable set NAME --body "value"`, or the repository Settings UI) needing the operator's own tenant ID and subscription ID (`az account show --query '{tenantId:tenantId, id:id}'`), neither of which is recorded anywhere in this repository — `design.md`'s Open Questions already names "which subscription" as unresolved. `AZURE_CLIENT_ID` and `ACR_LOGIN_SERVER` are deliberately **not** in this task — task 8.2 sets those, after they exist.
...
- [ ] 8.2 Set `AZURE_CLIENT_ID` (the platform state's `ci_identity_client_id` output), `ACR_LOGIN_SERVER` (its `acr_login_server` output), `BACKEND_IDENTITY_CLIENT_ID` (its `backend_identity_client_id` output) and `KEY_VAULT_NAME` (its `key_vault_name` output) as repository Variables — `design.md` D4's extension. All four come from the same platform-state apply and the same timing: runs after 8.1, the only source of any of them, and before 8.8's deploy, the first consumer of all four; matching this section's "ordering is not optional" framing. The other four CI variables are task 6.7, already set before this section runs.
