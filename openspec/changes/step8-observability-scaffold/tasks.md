...
- **9-d — Get the pipeline showing real data end-to-end** via `docker-compose up` and debug any gaps (missing spans, empty Prometheus targets, no Grafana data, missing trace/log correlation) collaboratively rather than solo.
  - [ ] Record the observed results as a "Verified End-to-End" section in `docs/step8.md` (Task 3 of `docs/step8-task9c9d.md`) — not started, but **no longer blocked**. Use the controlled run of 2026-08-19 (60/60 successes, no sleep during the run), not the contaminated 2026-08-12 one.
