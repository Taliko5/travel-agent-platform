...
- **Whether the log requirements are scoped too widely.** `Trace and Log Correlation for a Single Request` requires every log line a request emits to carry a `trace_id`, and the scaffold's `Structured Logging with Trace Correlation` requires backend logs to be JSON. Uvicorn's access logs are neither, and they are emitted while a request is being handled. Either both requirements mean the application's own log lines and should say so, or the system does not satisfy them. **Still open after pass 1 (2026-08-21).** Item 6 of the 9-d verification is the item that should settle this, and it does not: as written it names and checks only two application log lines, not "each log line it emits". Item 8's evidence, gathered for the separate purpose of recording log-format interleaving, incidentally shows a uvicorn access line for the same request with no `trace_id` — suggestive, but not the deliberate output of a test built to answer this question. The evidence this decision needs has not been produced. `tasks.md` records the gap in Item 6's method.
