...
`Trace and Log Correlation for a Single Request`'s original wording — "each log line it emits SHALL carry a non-null `trace_id`" — contradicted the scaffold's own `Structured Logging with Trace Correlation`, which explicitly permits a null `trace_id` for a log emitted outside any active span, "rather than causing an error." Pass 1b of the 9-d verification (2026-08-21) found exactly that case on a real request: `uvicorn`'s access line for a `/chat` request is emitted after the ASGI app returns, outside the request's span, and it is the only one of seventeen log lines that request produced without the request's `trace_id`. The other sixteen — application-logger JSON and OTel span-export blocks alike — all carried it, and all agreed on its value.
