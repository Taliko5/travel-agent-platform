...
`backend/api/main.py`'s `/chat` handler (lines 102-138) wraps `await graph.ainvoke(...)` in a `try`/`finally` that always records `chat_request_duration_seconds` and a `chat request completed` log line, with `intent` defaulting to `"unknown"` and `status` defaulting to `"error"` until the graph call succeeds. Nothing today bounds how long that `await` can take. On 2026-08-12, an outbound call to Gemini inside the graph hit `google_genai`'s internal `tenacity` retry loop under a failing network condition and the request ran for 1025.74 seconds before the client gave up — the retry loop, not `/chat`, was what eventually stopped it. The network condition that triggered that specific run is tracked separately in `docs/step8-host-network.md`; this change addresses the fact that `/chat` itself imposed no ceiling, which would let any future slow-or-hanging upstream call reproduce the same symptom regardless of cause.
