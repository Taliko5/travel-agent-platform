...
`backend/observability/metrics.py` currently declares and seeds `chat_request_duration_seconds` (histogram) and `intent_classification_total` (counter), with `llm_call_duration_seconds` and `rag_retrieval_total` declared but unrecorded. Its module docstring and `docs/plan.md` both describe "the four key Step 8 metrics." The error-rate panel of `observability/grafana/dashboards/travel-agent-overview.json` computes that rate by filtering `chat_request_duration_seconds_count{status="error"}`, and its `description` explicitly accepts that slow failures fall out of the latency-percentile panel because the error-rate panel still counts them by status. That the label takes no third value is a requirement, not an incidental property: `openspec/changes/step8-metric-design/specs/observability/spec.md` states it as `Scenario: No third status value is ever produced`.
...
Rationale, not a preference: the error-rate panel's expression filters `status="error"` exactly, and the latency-percentile panel's `description` accepts losing slow failures *because the error-rate panel still counts them* by status. `Scenario: No third status value is ever produced` in `openspec/changes/step8-metric-design/specs/observability/spec.md` is what that reasoning rests on. Introducing `status="timeout"` would silently remove timeouts from the error-rate panel that exists specifically to catch them.
