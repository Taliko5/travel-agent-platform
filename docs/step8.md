...
**What this check does not reach.** It was performed against the run named above, and work landed after it. `step8-metric-design`'s decisions D14–D19, dated 2026-08-22 in that change's `design.md`, gave `llm_call_duration_seconds` and `rag_retrieval_total` their recording call sites and added the two dashboard panels that read them — none of which existed when the conformance check ran, so none of it was covered. Two consequences follow for how the section above should be read: the finding that `llm_call_duration_seconds` was absent from `/metrics` describes an instrument that had no recording call site at the time and now has one, and the panel-query item covered the panel set as it then stood. The decision was to defer re-verification rather than skip it — it is outstanding, and nothing here asserts that the uncovered work behaves correctly or incorrectly. Re-running it means re-running the procedure in `openspec/changes/archive/2026-08-25-step8-metric-design/tasks.md` Section 2 against fresh traffic.
